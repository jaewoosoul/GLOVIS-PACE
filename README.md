...
- 세계지도 기반 선대 지도(D3 렌더러) — 8척 시뮬레이션 배경 선대 + GLOVIS A~E 5척은 시뮬레이션
  시계에 따라 실제 항로를 따라 이동
...
| `/map` | 선대 지도 | 8척 시뮬레이션 선대를 전체화면 지도에 표시, GLOVIS A~E는 시뮬레이션 시계에 따라 이동 |
...
2. `/dashboard`에 진입한다 — KPI는 자사 선단(GLOVIS A~E 5척) 기준으로 바로 계산되지만, 판단 대기열·
   최근 경보·최근 결정은 아직 아무것도 없는 빈 상태다(고정 Mock이 없기 때문).
3. `/map`에서 8척 전체 선대와 GLOVIS A~E의 위치를 확인한다. 상단 시뮬레이션 시계 바의
...
- `backend/`는 뉴스/RTA 분석 전용 API이며, 선박 위치를 위한 실시간 AIS 연동은 없다. 지도의 8척
